...
as in the previous example program that is not a major concern; bit if you
are manipulating or processing the strings in significant ways you probably
want to convert the strings to UCS-4 strings when using a Fortran compiler
...
constants is typically all that is allowed or compiler errors occur
when unsupported characters outside the Fortran character set are used
in the body of the code.

Unless the vendor extends Fortran beyond the requirements of the
Fortran standard.
...
+ [NEXT](https://github.com/lockstockandbarrel/earth/blob/main/docs/backslash_ext.md)
