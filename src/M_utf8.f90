...
public :: utf8_to_ucs4
public :: ucs4_to_utf8
public :: utf8_to_ucs4_via_io
public :: ucs4_to_utf8_via_io
...
!
! converting UCS-4 (representing Unicode code points) to UTF-8
...
!
! Representing UCS-4 in Fortran
!
!    Use the selected_char_kind('ISO_10646') intrinsic to define a kind
!    for UCS-4 characters.
!
!    You can declare character variables with this kind, e.g.,
!    character(kind=ucs4, len=<length>) :: my_ucs4_string.
!
!    Individual Unicode code points can be represented using hexadecimal
!    constants and the char function with the specified kind. For example,
!    char(int(z'4F60'), ucs4) represents the Chinese character '你' (Ni).
!
! Conversion Algorithm (Conceptual Example)
!
! You iterate through the UCS-4 string (conceptually treating each
! character as a 4-byte integer) and apply the UTF-8 encoding rules based
! on the code point's value. You then build a new byte array or
! a character string with character(len=1) elements, representing the
! UTF-8 sequence.
!
...
!    add a BOM.
!
!    The example doesn't explicitly handle surrogate pairs, which are
...
integer, dimension(4)                   :: utf8_code
  ...
  do i = 1, len(ucs4_string) ! iterate through each UCS-4 character.

    code_point = iachar(ucs4_string(i:i), kind=ucs4) ! Get the integer value of the UCS-4 char

    ! The if/else if statements check the code point value and determine
